...
def create_integration(team_id: UUID, data: dict) -> TeamIntegration:
    integration = TeamIntegration(team_id=team_id, slack_webhook_url=data.get("slack_webhook_url"), discord_webhook_url=data.get("discord_webhook_url"), enabled_triggers=data.get("enabled_triggers", {}),) # type:ignore
    db.session.add(integration)
    db.session.commit()
    return integration

def update_integration(integration: TeamIntegration, data: dict) -> TeamIntegration:
    allowed_fields = ['slack_webhook_url', 'discord_webhook_url', 'enabled_triggers']
